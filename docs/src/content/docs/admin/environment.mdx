...
## Download pipeline tuning

| Variable | Purpose |
| -------- | ------- |
| `AURRAL_PIPELINE_CONCURRENCY` | Override the number of tracks the Download Pipeline Worker processes in parallel. Default `4` (max `16`). You can also set this in the web UI under **Settings > Download clients > Download pipeline > Concurrent tracks**, which takes precedence at runtime. Increasing it can clear large download queues faster, but each active search holds a connection to the download client (for example, slskd), so very high values can overload it. Most installs want `2` to `8`. |
